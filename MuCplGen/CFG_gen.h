...
namespace MuCplGen
{
	...
	struct ParseRule
	{
		...
		void SetSemanticErrorAction(std::function<std::any* (std::vector<std::any*>)> on_error) { semantic_error = on_error; }
		...
		static Arg GetArg(const std::vector<std::any*>& data, int index)
		{
			...
			{
				if (typeid(Empty) == typeid(Arg)) return reinterpret_cast<Arg>(Empty{});
				else throw(std::exception("error"));
			}
			else return std::any_cast<Arg>(*data[index]);
		}
